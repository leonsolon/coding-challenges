...
def solution(A, B):
    if len(set(B)) == 1:
        return len(B)
    stream = []
    for size, direction, index in zip(A, B, range(len(B))):
        if direction == 0:
            if not stream:
                stream.append((size, direction, index))
            else:
                while stream and stream[-1][0] < size and stream[-1][1] == 1 and stream[-1][2] < index:
                    stream.pop()
                if not stream:
                    stream.append((size,direction,index))
                else:
                    if stream[-1][0] > size and stream[-1][1] == 1:
                        continue
                    else:
                        stream.append((size,direction,index))
        if direction == 1:
